...
export async function readConfiguration(c: PoolClient, id: string): Promise<Configuration> {
  ...
  const requirements = await c.query('select id,source_card_id,source_pair_id,required_card_id,min_in_deck from deck_requirements where deck_id=$1 order by id', [id]);
...
export async function writeConfiguration(c: PoolClient, id: string, data: Configuration): Promise<void> {
  ...
  for (const table of ['deck_cards', 'deck_starters', 'deck_requirements', 'deck_flags']) {
  ...
  for (const r of data.requirements) await c.query('insert into deck_requirements (deck_id,id,source_card_id,source_pair_id,required_card_id,min_in_deck) values ($1,$2,$3,$4,$5,$6)', [id,r.id,r.source_card_id,r.source_pair_id,r.required_card_id,r.min_in_deck]);
