...
function applySchema() {
  ...
  const files = ['db/schema.sql', 'db/migrations/001-deck-configuration.sql', 'db/migrations/002-profiles-and-conditions.sql', 'db/migrations/003-purge-legacy.sql', 'db/migrations/004-side-plans.sql', 'db/migrations/005-backoffice.sql'];
  for (const f of files) {
    const r = psql(readFileSync(path.join(ROOT, f), 'utf8'));
    if (r.status !== 0) throw new Error(`${f} : ${r.stderr.trim()}`);
  }
  applyCards();
  log('schéma, migrations 001 à 005 et cartes synthétiques appliqués');
}
