...
| 3 | Sous-domaine **admin.scratchrecode.com**, protégé par la connexion admin ; en production, service `admin` dans `deploy/docker-compose.prod.yml`, réseau `edge`, **aucun `ports:`**, joint par un bloc du Caddyfile goldfish (documenté, pas appliqué) ; en-têtes de sécurité et `noindex`. |
